...
class WifiAwareForegroundService : Service() {
    ...
    private val CHANNEL_ID = "WifiAwareChannel"
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        Log.d("onStartCommand","inside")
        startForeground(NOTIFICATION_ID, createNotification())
        startForegroundService(publishUsingWifiAware())
        startForegroundService(subscribeToWifiAwareSessions())

        val attachCallback = object : AttachCallback() {
            override fun onAttached(session: WifiAwareSession) {
                // WiFi Aware session is attached, handle your functionality here.
            }

            override fun onAttachFailed() {
                // Handle attach failure.
                stopSelf() // Stop the service if attachment fails.
            }
        }

        // Start your WiFi Aware functionality here
        if (wifiAwareManager.isAvailable) {
            wifiAwareManager.attach(attachCallback, Handler(Looper.getMainLooper()))
        } else {
            Log.d("WifiAwareForegroundService", "WiFi Aware is not available on this device.")
            stopSelf() // Stop the service if WiFi Aware is not available.
        }
    ...
    override fun onDestroy() {
        super.onDestroy()
        // Clean up and release resources when the service is stopped.
    }

    private fun createNotification(): Notification {
        Log.d("createNotification","inside")
        val notificationManager = getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(
                CHANNEL_ID,
                "Wifi Aware Service Channel",
        ...
        val notification = NotificationCompat.Builder(this, CHANNEL_ID)
            ...
            .setSmallIcon(androidx.appcompat.R.drawable.abc_ab_share_pack_mtrl_alpha) // Replace with your icon
            .setColor(ContextCompat.getColor(this, R.color.purple_200)) // Replace with your color
    ...
    fun subscribeToWifiAwareSessions(): Intent? {
        if (wifiAwareSession == null) {
            Log.d("WifiAwareForegroundService", "SUBSCRIBE: Wifi Aware session is not available")
            return null
        }
        ...
        val discoverySessionCallback = object : DiscoverySessionCallback() {
            override fun onSubscribeStarted(session: SubscribeDiscoverySession) {
                ...
                Log.d("WifiAwareForegroundService", "SUBSCRIBE: Subscribe started")
            }
            ...
            override fun onServiceDiscovered(
                ...
                if (peerHandle != null) {
                    ...
                    subscribeMessageLiveData.value =
                        "SUBSCRIBE: Connected to  $peerHandle: ${serviceSpecificInfo.toString()} ${matchFilter.toString()}"
                    val byteArrayToSend =
                        "tag_id:\"SUBSCRIBE\" readings:{tag_id:\"20\"  device_id:\"21\"  rssi:69  ts:{seconds:1696500095  nanos:85552100}}"
            ...
            override fun onMessageReceived(peerHandle: PeerHandle, message: ByteArray) {
                ...
                subscribeMessageLiveData.value =
                    "SUBSCRIBE: MessageReceived from $peerHandle message: ${message.decodeToString()}"
                ...
                val callback = object : ConnectivityManager.NetworkCallback() {
                    ...
                    override fun onCapabilitiesChanged(
                        ...
                        Log.d(
                            ...
                            "SUBSCRIBE onCapabilitiesChanged incoming: $networkCapabilities"
                        )
                    ...
                    override fun onAvailable(network: Network) {
                        if (currentNetworkCapabilities != null) {
                            val peerAwareInfo =
                                currentNetworkCapabilities?.transportInfo as WifiAwareNetworkInfo
    ...
    fun publishUsingWifiAware(): Intent? {
        if (wifiAwareSession != null) {
