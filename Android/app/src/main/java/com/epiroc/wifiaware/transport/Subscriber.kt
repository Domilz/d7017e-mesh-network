...
class Subscriber (
    ...
    private val _client: com.epiroc.wifiaware.lib.Client,
    private val _serviceName: String,
) {

    private val _serviceUUID = _client.getClientName().toByteArray()
    private val _peerHandleQueue = ArrayDeque<PeerHandle>()
    private var _currentPeerHandle: PeerHandle? = null
    private val _responseTimeout = 10000L // 10 seconds for example
    ...
    fun subscribeToWifiAwareSessions() {
            ...
            override fun onServiceDiscovered(
                ...
                if (peerHandle != null) {
                    ...
                    if (_peerHandleQueue.size == 1 && _currentPeerHandle == null) {
                        Log.d("Subscriber", "NEW PHONE: we are starting for a new phone because peerHandleQueue.size == 1 && currentPeerHandle == null in onServiceDiscovered ${_peerHandleQueue.size}")
        ...
        // what if no permission? maybe handle
    }
    ...
    fun createNetwork(peerHandle : PeerHandle, context : Context) {
        ...
        _networkCallbackSub = object : ConnectivityManager.NetworkCallback() {
            ...
            override fun onLost(network: Network) {
                ...
                Log.d("Subscriber", "SUBSCRIBE: Network lost for peer: $peerHandle")
            }
        }
    ...
    private fun handleDataExchange(peerHandle: PeerHandle, socket: Socket, connectivityManager: ConnectivityManager) {
        try {
            ...
            Log.d("Subscriber", "NEW PHONE: we are starting for a new phone because the information is already sent to the prev one in handleDataExchange ${_peerHandleQueue.size}")
            processNextPeerHandle()
        }
    }
        ...
        _responseTimer = Timer().apply {
            schedule(object : TimerTask() {
                override fun run() {
                    ...
                    synchronized(_peerHandleQueue) {
                        _currentPeerHandle = null
                        if (_peerHandleQueue.isNotEmpty()) {
                            _peerHandleQueue.addLast(peerHandle)
                            Log.d(
                                "Subscriber",
                                "NEW PHONE: we are starting for a new phone because Response timeout in $peerHandle startResponseTimer ${_peerHandleQueue.size}"
                            )
                            processNextPeerHandle()
                        } else {
    ...
    private fun processNextPeerHandle() {
        synchronized(_peerHandleQueue) {
            //Log.d("1Wifi", "NEW PHONE: we are starting for a new phone ${peerHandleQueue.size}")
