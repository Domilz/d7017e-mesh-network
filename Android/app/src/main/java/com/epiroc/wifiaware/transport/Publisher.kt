...
class Publisher(
            ...
            override fun onAvailable(network: Network) {
                ...
                while (retryCount < maxRetries) {
                    try {
                        ...
                        Log.d("NETWORK WIFI", "PUBLISH: Connection successful")
                        CoroutineScope(Dispatchers.IO).launch {
    ...
    private fun handleClient(clientSocket: Socket?,deviceIdentifier: String ) {
        ...
        _client.insertSingleMockedReading("publish")
            ...
            try {
                ...
                if (size > 0) {
                    ...
                    try{
                        ...
                        _client.tagClient.insert(messageBytes)
                    }catch (e: Exception){
                        Log.d("Publisher", "Error in inserting in handleClient" + e.message.toString())
                    }
                    Log.d("INFOFROMCLIENT", "Received protobuf message: ${_client.tagClient.getReadableOfProvidedSerializedState(messageBytes)}")
                } else {
        ...
        Log.d("DONEEE", "PUBLISH: All information received we are done $_messagesReceived, ${_client.tagClient.deserializedState}")
        ...
        _utility.saveToFile(_context, _client.tagClient.serializedState)
